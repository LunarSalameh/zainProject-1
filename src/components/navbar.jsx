          ...
          <div className="flex items-center gap-2">
            {socials.map((social, index) => (
              <div key={index} className="bg-white p-2">
                <a href={social.path}>{social.src}</a>
              </div>
            ))}
          </div>
        </div>
              ...
              {index === indexList && openSubList && item.subList && (
                <div className="absolute left-0 top-full mt-2 z-10 bg-white border">
                  {item.subList.map((subItem, subIndex) => (
                    <div
                      key={subIndex}
                      className="flex flex-col py-2 px-6 hover:bg-gray-200 active:text-white active:bg-[#AB7442]"
                    >
              ...
              {index === indexList && openSubList && item.subList && (
                <div className="flex flex-col py-2 border w-full gap-2 ">
                  {item.subList.map((subItem, subIndex) => (
                    <div
                      key={subIndex}
                      className="flex flex-col px-4 active:text-white active:bg-[#AB7442]"
                    >
                      <a href={subItem.src}>{subItem.title}</a>
                    </div>
                  ))}
